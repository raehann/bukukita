 ...
 * SET UP DATABASE
 */
$db = mysqli_connect("localhost", "root", "", "book-store");
...
function createUser($data)
{
    ...
    if (mysqli_fetch_assoc($userAlready)) {
        setcookie("error_auth", "Username sudah digunakan, silahkan ganti username yang lain", time() + 1);
...
function createBook($data)
{
    ...
    $queryCreateBook = "INSERT INTO products(judul_buku, kategori_buku, harga_buku, pembuat_buku, deskripsi_buku, rilis_buku, gambar) VALUES('$judul_buku','$kategori_buku','$harga_buku','$pembuat_buku','$deskripsi_buku','$rilis_buku', '$fileName')";
...
function createCheckout($data)
{
    ...
    if (empty($no_telp)) {
        ...
        header("Location: checkout.php");
        return false;
    } else if (strlen($no_telp)  <= 10) {
        setcookie("error_validation_checkout", 'Nomor telephone tidak boleh kurang dari 10!', time() + 1);
        header("Location: checkout.php");
        return false;
    }
    ...
    $sqlCheckout = "INSERT INTO checkout VALUES(id_checkout,'$id_ongkir','$id_product','$nama','$nama_buku','$email','$no_telp','$alamat','$harga_buku','$tipe_pembayaran','pending','$tanggalCheckout')";
    ...
    $updateBuku = date("Y-m-d h:m:s");
     ...
     if($fileName == ''){
        ...
        move_uploaded_file($_FILES['gambar']['tmp_name'], "gambar/".$_FILES['gambar']['name']);
         ...
         $findBook = "UPDATE products SET judul_buku = '$judul_buku', kategori_buku = '$kategori_buku', harga_buku = '$harga_buku', pembuat_buku = '$pembuat_buku', deskripsi_buku = '$deskripsi_buku', rilis_buku = '$updateBuku', gambar = '$fileName' WHERE id_product = '$id'";
